...
## 2026-04-13 - [Avoiding Parameter Slice Cloning in Transpiler Functions]
**Learning:** In V 0.5.1, slice slicing (e.g. `s[1..]`) or explicit cloning (`s.clone()`) inside recursive AST visitors or function generators can trigger heavy heap allocations, particularly when arrays are populated with struct values. Pre-allocating unified slices with total capacity and using an index offset variable (e.g., `start_idx`) instead of mutating and cloning slices results in a significant performance win. Additionally, sequential loops over static slice fields are significantly faster and cleaner than allocating temporary merged arrays just for iteration.
**Action:** Avoid mutating and slicing/cloning arrays inside transpiler generation routines. Use index-based traversal (e.g., `start_idx`) and direct sequential loop chains over individual properties instead.

## 2026-04-15 - [Deep-nested Multi-conditional Branching vs Compiler flat-match optimizations under -prod]
**Learning:** Under `-prod` optimization, replacing flat string `match` or equality checks with deep nested conditional byte/character comparisons (with multiple `&&`) can surprisingly result in slightly slower performance (~10% regression). This occurs because modern C compilers (like GCC or Clang) optimize flat-match/switch structures extremely effectively using SSE vector instructions or optimized jump tables, whereas complex multi-conditional byte checks introduce additional branch prediction overhead.
**Action:** Keep keyword match blocks flat and rely on C-level compiler optimization rather than introducing multiple nested byte comparison branches.

## 2026-04-15 - [Zero-Allocation Fast-Path for Capitalized Sum Type Names]
**Learning:** In V 0.5.1, `strings.new_builder` and its string buffer allocations incur noticeable heap allocation and GC pressure. For hot-path string formatting routines (like `get_sum_type_name`), introducing an early-return fast-path for already formatted strings (such as pre-capitalized clean names) to return basic string concatenation bypasses strings.Builder instantiation entirely and achieves a massive ~45.6% speedup.
**Action:** For helper formatting functions that construct prefixed string identifiers, always check if the input is already correctly capitalized/formatted and return a basic string concatenation to bypass strings.Builder allocations.
