...
## 2026-04-15 - [Zero-Allocation String Literal Scanning via Source Slicing]
**Learning:** Reconstructing scanned string literal tokens by joining separate prefixes, quotes, and scanned characters (e.g., using string interpolation like `${prefix}${q_str}${value}${q_str}`) results in multiple heap allocations, string copying, and performance bottlenecks. Since the entire string token is already present in the source string, slicing the full token directly using `l.source[start_idx..l.pos]` completely eliminates intermediate copying and heap allocations.
**Action:** Always prefer slicing entire parsed tokens directly from the input stream once boundary coordinates are identified, rather than manually reconstructing them through string interpolation or concatenation.
