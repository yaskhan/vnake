...
fn (mut p Printer) visit_function_def(node &FunctionDef) {
	...
	for i, dec in node.decorator_list {
		p.write(p.indent())
		walk_expr(mut p, dec)
		if i < node.decorator_list.len - 1 {
			p.write(',')
		}
		p.write('\n')
	}
