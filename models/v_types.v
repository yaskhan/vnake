...
pub fn map_python_type_to_v(py_type string, self_name string, allow_union bool, generic_map map[string]string, sum_type_registrar fn(string) string, literal_registrar fn([]string) string, tuple_registrar fn(string) string) string {
		...
		if clean_type.contains('fallback=') {
			...
			if clean_type.contains('fallback=') {
				...
				if parts.len > 1 {
					...
					end_idx := if comma_idx >= 0 && (bracket_idx < 0 || comma_idx < bracket_idx) { comma_idx } else { bracket_idx }
					if end_idx >= 0 {
	...
	if clean_type.contains('[') {
		...
		return _map_complex_type(clean_type, self_name, allow_union, generic_map, sum_type_registrar, literal_registrar, tuple_registrar)
	}

	// Fallback to basic type mapping
	return _map_basic_type(clean_type)
}

// _map_complex_type обрабатывает сложные типы
fn _map_complex_type(py_type string, self_name string, allow_union bool, generic_map map[string]string, sum_type_registrar fn(string) string, literal_registrar fn([]string) string, tuple_registrar fn(string) string) string {
	...
	for arg in args {
		mapped_args << map_python_type_to_v(arg.trim_space(), self_name, allow_union, generic_map, sum_type_registrar, literal_registrar, tuple_registrar)
	}
	...
	match base_type {
		...
		'Callable', 'typing.Callable', 'callable', 'collections.abc.Callable' {
			if args.len == 2 {
				arg_types := []
				ret_type := map_python_type_to_v(args[1].trim_space(), self_name, allow_union, generic_map, sum_type_registrar, literal_registrar, tuple_registrar)
...
fn _map_basic_type(name string) string {
	mut clean_name := name
