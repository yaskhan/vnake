...
pub fn apply_generic_arguments(
	...
	for i, tv in tvars {
		...
		// TODO: здесь mypy использует tv.id, у нас пока имя
	...
	if expanded is CallableType {
		...
		new_callable.variables = []TypeVarLikeType{} // В Mypy TypeVars очищаются после применения параметров
		return new_callable
	}
...
pub fn get_method_type_vars(info &TypeInfo) []TypeVarLikeType {
	mut vars := []TypeVarLikeType{}
	if tp := info.defn {
		// Извлечение TypeVars из определения класса (ClassDef)
		for tv in tp.type_vars {
			vars << tv
		}
	}
	return vars
