...
pub fn calculate_class_abstract_status(typ &TypeInfo, is_stub_file bool, mut errors Errors) {
	...
	if is_stub_file {
		...
		if abstract.len > 0 && abstract_in_this_class.len == 0 {
			...
			errors.report(typ.base.ctx.line, typ.base.ctx.column, 'Class ${typ.fullname} has abstract attributes ${attrs}', .error, none)
			errors.report(typ.base.ctx.line, typ.base.ctx.column, "If it is meant to be abstract, add 'abc.ABCMeta' as an explicit metaclass", .note, none)
		}
	}
	
	if typ.is_final && abstract.len > 0 {
		attrs := sorted_abstract.map('"${it}"').join(', ')
		errors.report(typ.base.ctx.line, typ.base.ctx.column, 'Final class ${typ.fullname} has abstract attributes ${attrs}', .error, none)
	}
}
		...
		for base_type in info.bases {
			if base_type.typ != none {
				if !base_type.typ.is_protocol && base_type.typ.fullname != 'builtins.object' {
					errors.report(info.base.ctx.line, info.base.ctx.column, 'All bases of a protocol must be protocols', .error, none)
				}
			}
		}
	}
		...
		if target_sym := module_names[target_name] {
			if target_info := target_sym.node {
				if target_info is TypeInfo {
					mut_info.bases << Instance{typ: target_info}
				}
			}
		}
	}
