...
pub fn (mut v TypeArgumentAnalyzer) visit_type_alias_type(t &TypeAliasType) !string {
	...
	alias := t.alias or {
		return error('TypeArgumentAnalyzer: Unfixed type alias ${t.alias_name}')
	}
		...
		if tvar is TypeVarType {
			...
			if tvar.values.len > 0 {
				...
				if arg is TypeVarType {
					if arg.values.len == 0 {
						is_error = true
						v.fail('Invalid TypeVar "${arg.name}" as type argument for "${name}"', context, code_type_var)
						continue
					}
			...
			if p_arg !is ParamSpecType && p_arg !is ParametersType && p_arg !is AnyType {
				is_invalid = true
				v.fail('Can only replace ParamSpec with a parameter types list or another ParamSpec, got ${p_arg.type_str()}', context, valid_type)
			}
		}
	}
...
pub fn (mut v TypeArgumentAnalyzer) check_type_var_values(name string, actuals []MypyTypeNode, arg_name string, valids []MypyTypeNode, context Context) bool {
	...
	for actual in actuals {
		...
		if p_actual is AnyType || p_actual is UnboundType { continue }
		...
		if !found {
			is_err = true
			v.fail('Incompatible value for TypeVar "${arg_name}" in "${name}"', context, code_type_var)
		}
	}
	return is_err
}
