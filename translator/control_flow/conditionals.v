...
fn (mut m ControlFlowModule) collect_narrowing(node ast.Expression, positive bool) map[string]string {
	...
	if node is ast.Call {
					...
					if positive {
						...
						if base_type.contains('|') || base_type.starts_with('SumType_') {
							...
							if base_type in ['SumType_IntString', 'int | string'] {
								res[var_name] = if tg.narrowed_type == 'int' {
									'string'
								} else {
									'int'
								}
		...
		if left is ast.Name {
			var_name := left.id
			is_none := (right is ast.Constant && right.value == 'None')
				|| (right is ast.Name && right.id in ['None', 'none'])
				|| right is ast.NoneExpr
			if is_none {
				if (op == 'is not' && positive) || (op == '!=' && positive)
					|| (op == 'is' && !positive) || (op == '==' && !positive) {
				...
				} else if (op == 'is' && positive) || (op == '==' && positive)
					|| (op == 'is not' && !positive) || (op == '!=' && !positive) {
					res[var_name] = 'none'
				}
			}
		}
	...
	for var_name, n_type in narrowing {
		...
		if branch_suffix != '_while' {
			if base_type.starts_with('?') && (narrowed_type == base_type[1..]
				|| '&' + narrowed_type == base_type[1..]
				|| narrowed_type == '&' + base_type[1..]) {
				is_auto = true
			} else if (base_type.contains('|') || base_type.starts_with('SumType_'))
				&& !narrowed_type.contains('|') {
...
fn (mut m ControlFlowModule) visit_if_inner(node ast.If, is_elif bool) {
	if !is_elif {
		...
		else_vars := if node.orelse.len > 0 {
			m.collect_assigned_vars(node.orelse)
		} else {
			[]AssignedVar{}
		}
		...
		for vinfo in if_vars {
			var := vinfo.name
			if var.contains('.') || m.is_declared_local(var) {
				continue
			}
			...
			if !v_type.starts_with('?') && v_type != 'Any' {
				v_type = '?' + v_type
			}
		...
		for vinfo in else_vars {
			var := vinfo.name
			if var.contains('.') || m.is_declared_local(var) || var in seen_vars {
				continue
			}
...
fn (m &ControlFlowModule) is_type_checking(node ast.If) bool {
	if node.test is ast.Name && node.test.id == 'TYPE_CHECKING' {
		return true
	}
	return false
}
