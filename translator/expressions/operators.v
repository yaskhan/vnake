	...
	match op {
		...
		'**' {
			...
			if node.right is ast.UnaryOp && node.right.op.value == '-' {
				if node.right.operand is ast.Constant {
		...
		'%' {
			if left_type == '[]u8'
				|| (node.left is ast.Constant && node.left.value.starts_with('b"')) {
				eg.state.used_builtins['py_bytes_format'] = true
				return eg.format_percent_bytes(left, node.right)
			}
			is_string_fmt := left_type == 'string' || left_type == 'LiteralString'
				|| (node.left is ast.Constant && node.left.value.starts_with('"'))
			if is_string_fmt {
				...
				if node.left is ast.Constant {
					...
					if !fmt_str.contains('%%') && !fmt_str.contains('%(') && fmt_str.count('%') > 0 {
						if mapped := eg.map_python_percent_to_v_interpolation(fmt_str,
							node.right)
						{
							return mapped
						}
...
fn (mut eg ExprGen) build_pythonic_bool_op(node ast.BinaryOp, is_and bool) string {
	...
	if is_and {
		return 'if ${l_cond} { ${r_expr} } else { ${l_expr} }'
	} else {
		return 'if ${l_cond} { ${l_expr} } else { ${r_expr} }'
	}
}
...
pub fn (mut eg ExprGen) visit_bool_op(node ast.BoolOp) string {
	...
	if node.values.len == 2 {
		...
		if is_and {
			return '(if ${l_cond} { ${r_val} } else { ${l_val} })'
		} else {
			return '(if ${l_cond} { ${l_val} } else { ${r_val} })'
		}
	}
...
pub fn (mut eg ExprGen) visit_compare(node ast.Compare) string {
	...
	if node.ops.len == 1 && comps.len == 2 {
		return eg.translate_single_comparison(comps[0], node.ops[0].value, comps[1], node.left,
			node.comparators[0])
	}
	...
	for i, op in node.ops {
		...
		res := eg.translate_single_comparison(comps[i], op.value, comps[i + 1], left_node,
			right_node)
		parts << '(${res})'
	}
	return parts.join(' && ')
...
fn (mut eg ExprGen) translate_single_comparison(left string, op string, right string, left_expr ast.Expression, right_expr ast.Expression) string {
	...
	if op == 'in' || op == 'not in' {
		...
		if is_none_expr(left_expr) {
			if right_type.starts_with('map[') {
				return '${prefix}(none in ${right})'
			}
