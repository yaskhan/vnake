...
pub fn (h FunctionsGenerationHandler) generate_function(node &ast.FunctionDef,
	...
	for arg in args {
		...
		if (a_clean.len > 0 && a_clean[0].is_capital()
			&& a_clean !in ['Any', 'LiteralString', 'bool', 'int', 'f64', 'string', 'void', 'LiteralEnum_', 'NoneType']
			&& !a_clean.starts_with('SumType_') && !a_clean.starts_with('TupleStruct_')
	...
	if !is_generator {
		...
		if !is_v_native_method && r_clean_ptr.len > 0 && r_clean_ptr[0].is_capital()
			&& r_clean_ptr !in ['Any', 'LiteralString', 'bool', 'int', 'f64', 'string', 'void', 'LiteralEnum_', 'NoneType']
			&& !r_clean_ptr.starts_with('SumType_') && !r_clean_ptr.starts_with('TupleStruct_')
