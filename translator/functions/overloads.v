...
pub fn generate_overload_variants(node &ast.FunctionDef,
	struct_name string,
	is_method bool,
	...
	for sig in sigs {
		...
		for arg_name, arg_type in sig {
			...
			clean_type = clean_type.replace('?', 'opt_').replace('[]', 'arr_').replace('Iterable[T]',
				'arr_generic').replace('Iterable', 'arr_generic').replace('[', '_').replace(']',
				'').replace('.', '_')
		...
		mut func_generics := extract_implicit_generics(node, state.type_vars, state.paramspec_vars,
			map[string]bool{}, state.current_class_generics, sanitize_fn)
		...
		for k, v in v_gen_map {
			combined_gen_map[k] = v
		}
		...
		if is_method && (dec_info.is_classmethod || dec_info.is_staticmethod) {
			for cg in state.current_class_generics {
				...
				if v_gen !in v_gens_to_declare {
					v_gens_to_declare << v_gen
				}
			}
		}

		gen_s := base.get_generics_with_variance_str(v_gens_to_declare, combined_gen_map,
			state.generic_variance, state.generic_defaults)
		...
		if (dec_info.is_classmethod || dec_info.is_staticmethod) && sig_ret.len > 0
			&& sig_ret[0].is_capital() && sig_ret !in ['Any', 'LiteralString', 'bool', 'int', 'f64'] {
