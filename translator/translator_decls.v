...
fn (mut t Translator) visit_function_def(node &ast.FunctionDef) {
	...
	mut env := functions.new_function_visit_env(t.state, t.analyzer, fn [mut t] (stmt ast.Statement) {
		t.visit_stmt(stmt)
	...
	if func_code.len > 0 {
		...
		is_lifted := node.type_params.len > 0
			|| (prev_func.len > 0 && !func_code.contains(' := fn '))
...
fn (mut t Translator) visit_class_def(node &ast.ClassDef) {
	...
	mut env := classes.new_class_visit_env(t.state, t.analyzer, fn [mut t] (stmt ast.Statement) {
		t.visit_stmt(stmt)
	}, fn [mut t] (expr ast.Expression) string {
		return t.visit_expr(expr)
	}, fn [mut t] (s string) {
